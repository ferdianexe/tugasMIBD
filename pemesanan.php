<?php
    ...
    if ($_SERVER['REQUEST_METHOD']=='POST') {
			if (isset($_SESSION['IS_UPDATING'])) {
				if (!is_null($_SESSION['IS_UPDATING'])) {
					if (isset($_POST['hari'])&&$_POST['hari']!=null&&isset($_POST['speciality']) &&$_POST['speciality']!=null&& isset($_POST['dokter']) && $_POST['dokter']!=null) {
						$idDokter = $_POST['dokter'];
						$tanggalTemu = $_POST['hari'];
						$idP = $_SESSION['IS_UPDATING'];
						$queryForUpdate = "UPDATE pekerjaandokter 
															SET idDokter='$idDokter', waktuTemu='$tanggalTemu'
															WHERE idPenanganan='$idP'";
						unset($_SESSION['IS_UPDATING']);
						$resultUpdate = $conn->query($queryForUpdate);
        ...
        // 1 pertemuan berkisar 30 menit
			else if (isset($_POST['dokter'])&&$_POST['dokter']!=null) {
		...
		// kalau methodnya GET -> untuk update
